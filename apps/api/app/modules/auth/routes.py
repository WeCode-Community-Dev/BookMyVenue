...
from app.modules.auth.schemas import AuthMeResponse, ForgotPasswordRequest, SendConfirmationRequest
...
@router.post("/send-confirmation", status_code=204)
def send_confirmation(
    ...
    client_ip = request.client.host if request.client else "unknown"
    limit = settings_store.get_setting(db, "send_confirmation_rate_limit_per_hour")
    enforce_ip_hourly_limit(client_ip, "send_confirmation", limit)

    service.send_signup_confirmation(body.email, body.redirect_to)
