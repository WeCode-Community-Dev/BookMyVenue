...
def search(
    ...
    enforce_per_minute_limit(db, auth.user_id, "deep_research")
    enforce_daily_limit(
        auth.user_id, "deep_research", settings_store.get_setting(db, "deep_research_daily_limit")
    )
...
async def trigger_external(
    ...
    """
    enforce_per_minute_limit(db, auth.user_id, "deep_research_external")
