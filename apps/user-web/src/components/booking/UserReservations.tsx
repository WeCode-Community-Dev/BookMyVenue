...
export function UserReservations() {
  ...
  const { data: reservations = [], isLoading, isError, refetch } = useQuery<UserReservationResponse[]>({
    queryKey: ['my-external-reservations'],
    queryFn: () => deepResearchEndpoints(client).getMyReservations(),
  })

  if (isLoading) {
    return <div className="py-20"><LoadingScreen fullScreen={false} /></div>
  }
    ...
    return (
      <div className="py-10">
        <ErrorState
          ...
          action={<button onClick={() => void refetch()} className="rounded-xl bg-brand px-4 py-2 text-sm font-semibold text-white">Retry</button>}
        />
      </div>
    )
  }
      ...
      <div className="flex flex-1 flex-col justify-between">
        <div>
          <div className="mb-3 flex items-center justify-between">
            <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${statusInfo.bg} ${statusInfo.color} border ${statusInfo.border}`}>
              {statusInfo.stage === 4 ? <CheckCircle2 className="h-3.5 w-3.5" /> : <Clock className="h-3.5 w-3.5" />}
              {statusInfo.label}
            </span>
          ...
          <div className="mt-4 flex flex-wrap gap-4 text-sm text-zinc-600 dark:text-zinc-400">
            {res.event_date && (
              <div className="flex items-center gap-1.5">
                ...
                <span><span className="font-medium">Date:</span> {formatDate(res.event_date)}</span>
              </div>
            )}
            {res.guest_count && (
              <div className="flex items-center gap-1.5">
                <Users className="h-4 w-4 text-zinc-400" />
                <span><span className="font-medium">Guests:</span> {res.guest_count}</span>
              </div>
            )}
            {res.notes && (
              <div className="flex items-center gap-1.5">
                <FileText className="h-4 w-4 text-zinc-400" />
                <span><span className="font-medium">Notes:</span> {res.notes}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mt-6 border-t border-zinc-100 pt-5 dark:border-ink-800">
          ...
          <p className="mt-3 text-xs text-zinc-400 text-center">{statusInfo.desc}</p>
        </div>
      </div>
    </div>
  )
