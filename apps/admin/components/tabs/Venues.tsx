...
import {
    ...
    fetchPendingVenues,
    fetchApproved,
    approveVenue as approveVenueAction,
    rejectVenue as rejectVenueAction,
...
export function VenuesPage() {
    ...
    const [venueFilter, setVenueFilter] = useState<VerificationStatus | "All">("All");
    ...
    const fetchPage = async (filter: VerificationStatus | "All", p: number) => {
        let result;
        if (filter === "All") result = await fetchAllVenues(p, PAGE_SIZE);
        else if (filter === VerificationStatus.PENDING) result = await fetchPendingVenues(p, PAGE_SIZE);
    ...
    const handleApprove = async (id: string) => {
        ...
        try {
            ...
            setVenues((v) => v.map((x) => (x.id === id ? { ...x, status: VerificationStatus.APPROVED } : x)));
    ...
    const handleReject = async () => {
        ...
        try {
            ...
            setVenues((v) =>
                v.map((x) => (x.id === rejectTarget ? { ...x, status: VerificationStatus.REJECTED } : x)),
            );
            setPendingCount((c) => Math.max(0, c - 1));
                        ...
                        <div className="flex gap-2 flex-wrap">
                            {(
                                ...
                                <button
                                    ...
                                    className={`px-3 py-2 rounded-xl text-xs font-semibold transition-all ${venueFilter === s.value ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-secondary"}`}
                                >
                                    {s.label}
                                    {s.value === VerificationStatus.PENDING && pendingCount > 0 && (
                                        <span
                                            className={`ml-1.5   text-xs px-1 rounded-full border ${venueFilter === s.value ? "bg-primary text-primary-foreground " : "bg-muted text-muted-foreground hover:bg-secondary border-muted-foreground"}`}
                                        >
                                            {pendingCount}
                                        </span>
                                    )}
                        ...
                        <TableBody>
                            {pagedVenues.map((v) => (
                                <TableRow key={v.id}>
                                    ...
                                    <TableCell className="px-5 py-3.5">
                                        <span
                                            className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${v.isActive ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-muted text-muted-foreground border-border"}`}
                                        >
