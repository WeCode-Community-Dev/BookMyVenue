...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6 pb-8">
        ...
        <div className="flex flex-wrap gap-3">
          {quickActions.map((action, i) => (
            <Link
              key={i}
              to={action.link}
              className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-zinc-700 bg-white border border-zinc-200 rounded-lg hover:bg-zinc-50 hover:text-zinc-900 transition-colors shadow-sm"
            >
      ...
      {/* Header Strip */}
      <section>
        <h1 className="text-2xl font-semibold text-zinc-900 tracking-tight">Good to see you, {userName}</h1>
        <p className="mt-1 text-zinc-500">Here is what needs your attention today.</p>
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Performance Chart */}
        <Card className="lg:col-span-2 p-6 flex flex-col border-zinc-200 shadow-sm rounded-xl">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h3 className="font-semibold text-zinc-900">Performance Trends</h3>
              ...
              {['7D', '30D', '3M', '6M', '12M'].map(p => (
                <button
                  ...
                  className={`px-3 py-1 text-xs font-medium rounded-md transition-colors ${
                    p === timeRange ? "bg-white shadow-sm text-zinc-900" : "text-zinc-500 hover:text-zinc-700"
                  }`}
                >
                  {p}
                </button>
          ...
          <div className="grid grid-cols-3 gap-4 mb-8 px-2">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-sm text-zinc-500">Enquiries</span>
              </div>
        ...
        {/* Upcoming Events */}
        <Card className="p-6 flex flex-col border-zinc-200 shadow-sm rounded-xl">
          ...
          <div className="flex-1 space-y-5 overflow-y-auto">
            {upcomingEvents.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full py-8 text-center">
                ...
                <p className="text-xs text-zinc-400 mt-1">Confirmed bookings will appear here</p>
              </div>
            ) : (
                ...
                <div key={event.booking_id} className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-sm font-medium text-zinc-900 truncate">{eventTypeLabel(event.event_type)}</p>
                      <StatusBadge label="Confirmed" variant="success" className="text-[10px] px-1.5 py-0" />
                    </div>
                    <div className="flex items-center justify-between gap-2 mt-1">
                      <p className="text-xs text-zinc-500 truncate">{event.venue_name}</p>
                      <p className="text-xs text-zinc-400 whitespace-nowrap">{formatEventDate(event.starts_at)}</p>
                    </div>
                    <p className="text-xs text-zinc-400 mt-0.5">{event.guest_count} guests</p>
                  </div>
                </div>
              ))
            )}
