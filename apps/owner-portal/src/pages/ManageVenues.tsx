...
const STATUS: Record<StatusKey, { label: string; dot: string; pill: string }> = {
  ...
  draft:            { label: 'Draft',         dot: 'bg-zinc-400',    pill: 'bg-zinc-100 dark:bg-ink-800      text-zinc-600 dark:text-zinc-400     ring-zinc-300/40'    },
    ...
    <div className="space-y-6 pb-10">
            ...
            >
              ...
              {counts[t.id] > 0 && (
                <span className={`text-[11px] font-semibold rounded-full px-1.5 py-0.5 leading-none tabular-nums ${
                  filter === t.id ? 'bg-brand-100 text-brand-700' : 'bg-zinc-100 dark:bg-ink-800 text-zinc-500 dark:text-zinc-400'
                }`}>
                  {counts[t.id]}
                </span>
              )}
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {[1, 2, 3, 4, 5, 6].map(i => (
            <div key={i} className="rounded-xl overflow-hidden border border-zinc-200 dark:border-ink-700 bg-white dark:bg-ink-900">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {filteredVenues.map(venue => {
            ...
            return (
              <div
                key={venue.id}
                className="card-enter group flex flex-col rounded-xl border border-zinc-200 dark:border-ink-700 bg-white dark:bg-ink-900 overflow-hidden shadow-sm hover:shadow-md hover:border-zinc-300 dark:border-ink-700 dark:hover:border-ink-700 transition-all duration-200"
              >
                ...
                >
                  {coverPhoto ? (
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center gap-2 bg-zinc-50 dark:bg-ink-800">
                      ...
                      <span className="text-xs text-zinc-400 dark:text-zinc-400">No photos yet</span>
                    </div>
                  )}
                ...
                <div className="flex flex-col flex-1 p-4 gap-3">
                  ...
                  <div>
                    ...
                    <div className="mt-1.5 flex items-center gap-3 text-xs text-zinc-400 dark:text-zinc-400">
                  ...
                  {isDraftIncomplete && (
                    <div className="space-y-1">
                      <div className="flex justify-between text-xs text-zinc-400 dark:text-zinc-400">
                  ...
                  {/* Actions */}
                  <div className="flex items-center gap-2 pt-1 border-t border-zinc-100 dark:border-ink-700">
                    {isDraftIncomplete ? (
