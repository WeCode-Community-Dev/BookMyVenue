...
function previewMessage(message: string | null) {
  if (!message) return 'No messages yet — reply to start'
...
export default function Messages() {
  ...
  const activeConversation: Conversation | null = useMemo(() => {
    ...
    return {
      ...
      other_party_name: booking.user_full_name || 'Guest',
      last_message: null,
  ...
  const filtered = useMemo(() => {
    ...
    return conversations.filter(
      (c) =>
        ...
        (c.booking_status || '').toLowerCase().includes(q),
    )
    ...
    return (
      <div className="space-y-5 pb-8">
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-[340px_minmax(0,1fr)]">
          ...
          <div className="hidden h-[620px] animate-pulse rounded-xl bg-zinc-100 dark:bg-ink-800 lg:block" />
        </div>
      </div>
    )
  }
      ...
      <div className="space-y-5 pb-8">
        ...
        <section className="rounded-xl border border-dashed border-zinc-200 bg-zinc-50 px-6 py-16 text-center dark:border-ink-800 dark:bg-ink-900/60">
          ...
          <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-zinc-500 dark:text-zinc-400">
            When customers message you about a booking, conversations will appear here.
          </p>
        </section>
      </div>
    )
  ...
  return (
    <div className="space-y-5 pb-8">
      ...
      <div className="grid h-[calc(100vh-12rem)] min-h-[560px] grid-cols-1 gap-4 lg:grid-cols-[340px_minmax(0,1fr)]">
        <aside
          className={`${activeBookingId ? 'hidden lg:flex' : 'flex'} min-h-0 flex-col overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-ink-800 dark:bg-ink-900`}
        >
          <div className="border-b border-zinc-100 px-3 py-3 dark:border-ink-800">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search guests or venues…"
                    ...
                    <div className="flex gap-3">
                      <div
                        className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${
                          isActive
                            ? 'bg-brand text-white'
                            : 'bg-zinc-100 text-zinc-600 ring-1 ring-zinc-200/70 dark:bg-ink-800 dark:text-zinc-300 dark:ring-ink-700'
                        }`}
                      >
        ...
        <main className={`${!activeBookingId ? 'hidden lg:flex' : 'flex'} min-h-0 flex-col`}>
          {activeBookingId && activeConversation ? (
              ...
              <div className="flex shrink-0 flex-col gap-3 border-b border-zinc-100 px-4 py-3 dark:border-ink-800 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex min-w-0 items-center gap-3">
                  <button
                    ...
                    className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-zinc-200 bg-white text-zinc-600 shadow-sm transition-colors hover:bg-zinc-50 dark:border-ink-800 dark:bg-ink-950 dark:text-zinc-300 dark:hover:bg-ink-800 lg:hidden"
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <h2 className="truncate text-base font-semibold text-zinc-900 dark:text-zinc-100">
                        {activeConversation.other_party_name || 'Guest'}
                      </h2>
                      <StatusBadge
            ...
            <div className="flex h-full flex-col items-center justify-center rounded-xl border border-dashed border-zinc-200 bg-zinc-50 px-6 text-center dark:border-ink-800 dark:bg-ink-900/60">
              <MessageSquare className="mb-3 h-8 w-8 text-zinc-400" />
              <h3 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">
                Conversation unavailable
              </h3>
              <p className="mt-2 max-w-xs text-sm text-zinc-500 dark:text-zinc-400">
                Unable to load this booking conversation.
              </p>
              <button
                type="button"
                onClick={() => navigate('/messages')}
            ...
            <div className="hidden h-full flex-col items-center justify-center rounded-xl border border-dashed border-zinc-200 bg-zinc-50 px-6 text-center dark:border-ink-800 dark:bg-ink-900/60 lg:flex">
              ...
              <p className="mt-2 max-w-xs text-sm leading-6 text-zinc-500 dark:text-zinc-400">
                Choose a guest conversation to reply in real time.
              </p>
            </div>
          )}
        </main>
    ...
    <section className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-ink-800 dark:bg-ink-900">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="inline-flex w-fit items-center gap-2 rounded-full border border-zinc-200 bg-zinc-50 px-3 py-1.5 text-xs font-medium text-zinc-600 dark:border-ink-800 dark:bg-ink-950 dark:text-zinc-300">
          <span
            className={`h-2 w-2 rounded-full ${unreadTotal > 0 ? 'bg-brand' : 'bg-zinc-300 dark:bg-ink-600'}`}
          />
          {unreadTotal > 0
            ? `${unreadTotal} unread`
