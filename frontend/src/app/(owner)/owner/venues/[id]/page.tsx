...
export default function OwnerEditVenuePage({ params }: Props) {
  ...
  const checkIfSameTiming = (avail: any) => {
    if (!avail) return true;
    const days = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'];
    const openDays = days.filter(d => avail[d]?.isOpen);
    if (openDays.length <= 1) return true;
    
    const mondaySlotsStr = JSON.stringify(avail.monday?.slots || []);
    for (let i = 0; i < openDays.length; i++) {
      const dayKey = openDays[i];
      if (dayKey === 'monday') continue;
      const daySlotsStr = JSON.stringify(avail[dayKey]?.slots || []);
      if (daySlotsStr !== mondaySlotsStr) {
        return false;
      }
    }
    return true;
  ...
  // Auth checking and data fetching
  useEffect(() => {
    setIsClient(true);
    ...
    try {
      ...
      if (user.role !== 'Venue owner' && user.role !== 'Admin') {
        router.push('/');
      } else {
        setCurrentUser(user);
        fetchVenueDetails();
  ...
  const fetchVenueDetails = async () => {
    ...
    try {
      ...
      setName(venue.name || '');
      setType(venue.type || 'Conference Room');
      setCapacity(venue.capacity || 10);
      setLocation(venue.location || '');
      setPricePerHour(venue.pricePerHour ?? venue.hourlyBookingConfiguration?.pricePerHour ?? 50);
      setSelectedImages(venue.images || []);
      setDescription(venue.description || '');
      const loadedAmenities = venue.amenities || [];
      setAmenities(loadedAmenities);

      const uniqueCustom = loadedAmenities.filter((a: string) => !AMENITY_OPTIONS.includes(a));
      setAvailableAmenities([...AMENITY_OPTIONS, ...uniqueCustom]);

      // Load configurations
      setFixedSlots(venue.bookingModes?.fixedSlots ?? true);
      setHourlyBooking(venue.bookingModes?.hourlyBooking ?? false);
      setCustomRequests(venue.bookingModes?.customRequests ?? false);

      setHourlyStartTime(venue.hourlyBookingConfiguration?.startTime ?? '09:00');
      setHourlyEndTime(venue.hourlyBookingConfiguration?.endTime ?? '22:00');
      setHourlyMinHours(venue.hourlyBookingConfiguration?.minimumHours ?? 1);
      setHourlyMaxHours(venue.hourlyBookingConfiguration?.maximumHours ?? 12);

      setCustomOwnerApprovalRequired(venue.customBookingConfiguration?.ownerApprovalRequired ?? true);
      setCustomMinNoticeHours(venue.customBookingConfiguration?.minimumNoticeHours ?? 24);

      if (venue.availability) {
        const loadedAvailability: any = {};
        const days = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'];
        days.forEach((dayKey) => {
          const dayData = venue.availability[dayKey];
          loadedAvailability[dayKey] = {
        ...
        setSameTiming(checkIfSameTiming(loadedAvailability));
      } else {
        setAvailability({
          monday: { isOpen: true, slots: [] },
          tuesday: { isOpen: true, slots: [] },
      ...
      setErrorMsg('Failed to load venue details. Please try again.');
    } finally {
      setFetching(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err.response?.data?.message || 'Failed to update venue. Please check your inputs and try again.';
  ...
  if (!isClient || !currentUser || fetching) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="animate-pulse flex flex-col items-center gap-4">
          <div className="w-12 h-12 rounded-full border-4 border-indigo-600 border-t-transparent animate-spin"></div>
          <p className="text-slate-600 font-medium">Loading venue details...</p>
        </div>
      </div>
    );
  }
  ...
  const stepsList = [
    ...
    { number: 4, name: 'Amenities & Review', desc: 'Finalize changes' },
  ];
      ...
      <main className="flex-1 max-w-3xl w-full mx-auto px-4 sm:px-6 py-12">
        ...
        <div className="bg-white p-8 sm:p-10 rounded-3xl border border-slate-200 shadow-xl relative overflow-hidden">
          ...
          <div className="mb-8 border-b border-slate-100 pb-5">
            <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight">Edit Venue Details</h1>
            <p className="text-slate-500 mt-1 text-sm">
              Update the specifications, configurations, and pricing options for your venue.
            </p>
          </div>
          ...
          {success && (
            <div className="mb-6 p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm font-semibold flex items-center gap-2 animate-bounce">
              ...
              <span>Venue updated successfully! Redirecting to Venues list...</span>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="flex justify-center px-6 pt-5 pb-6 border-2 border-slate-200 border-dashed rounded-2xl bg-slate-50 hover:bg-slate-100/70 transition-all cursor-pointer relative group">
                      ...
                      <div className="space-y-1 text-center pointer-events-none">
                        ...
                        <div className="flex text-sm text-slate-650 font-semibold justify-center">
                          <span className="text-indigo-655 hover:text-indigo-500">Upload new files</span>
                    ...
                    {selectedImages.length > 0 && (
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-2">
                        {selectedImages.map((imgSrc, index) => {
                          ...
                          const displayUrl = isBase64 ? imgSrc : (imgSrc.startsWith('/uploads/') ? `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}${imgSrc}` : imgSrc);

                          return (
                            <div key={index} className="relative aspect-video rounded-xl overflow-hidden border border-slate-200 shadow-sm group">
                              <img src={displayUrl} alt={`Selected ${index + 1}`} className="w-full h-full object-cover" />
                              <button
                                type="button"
                ...
                <div className="space-y-6 animate-fadeIn">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div 
                      ...
                      className={`border-2 rounded-2xl p-5 cursor-pointer transition-all duration-200 flex flex-col justify-between ${
                        hourlyBooking 
                          ? 'border-indigo-655 bg-indigo-50/50 shadow-md shadow-indigo-100/50' 
                          : 'border-slate-200 bg-white hover:border-slate-300'
                      }`}
                    >
                      <div className="flex justify-between items-start">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-4">
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            className="h-4 w-4 rounded border-slate-300 text-indigo-655 focus:ring-indigo-500 cursor-pointer"
                          />
              ...
              {currentStep === 4 && (
                <div className="space-y-8 animate-fadeIn">
                  ...
                  <div className="border-t border-slate-200 pt-6 space-y-4">
                    <h3 className="text-base font-bold text-slate-850">Review Updated Details</h3>
            ...
            <div className="flex items-center justify-between pt-6 border-t border-slate-100 mt-8">
              {currentStep > 1 ? (
                ...
                <Link
                  href="/owner/venues"
                  className="px-6 py-2.5 border border-slate-250 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-550 transition-all cursor-pointer text-center"
                >
                  Cancel
                </Link>
              )}
