        ...
        <div className="absolute top-3 left-3 z-10 flex flex-wrap gap-1 max-w-[80%]">
          {venue.categories && venue.categories.length > 0 ? (
            venue.categories.map((cat) => (
              <Badge key={cat} variant="rose" className="shadow-xs backdrop-blur-md bg-white/90 border-slate-200/50 text-[10px] py-0.5 px-2 font-bold">
                {cat}
              </Badge>
            ))
          ) : (
            <Badge variant="rose" className="shadow-xs backdrop-blur-md bg-white/90 border-slate-200/50 text-[10px] py-0.5 px-2 font-bold">
              {venue.category}
            </Badge>
          )}
        </div>
