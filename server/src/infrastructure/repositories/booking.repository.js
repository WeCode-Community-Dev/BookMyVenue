...
export class BookingRepositoryImpl extends BookingRepository {
  ...
  async findAllFiltered(query) {
    ...
    if (query.search) {
      ...
      const vendorIds = await VendorModel.find({
        $or: [{ fullName: regex }, { companyName: regex }],
      }).distinct("_id");
      const venueIds = await VenueModel.find({ name: regex }).distinct("_id");
      const searchFilter = [
        { userId: { $in: userIds } },
      ...
      if (Types.ObjectId.isValid(query.search)) {
        searchFilter.push({ _id: new Types.ObjectId(query.search) });
      }
    ...
    const result = await BookingModel.aggregate([
      {
        $group: {
          ...
          rejectedBookings: {
            $sum: {
              $cond: [{ $eq: ["$status", BookingStatus.REJECTED] }, 1, 0],
            },
          },
  ...
  async hasOverlappingBooking(venueId, bookingDate, startTime, endTime) {
  ...
  async getUserBookings(userId, { page, limit, status, search, sortBy }) {
    const filter = { userId };
