...
export class VenueRepository extends IVenueRepository {
    ...
    async findByVendorAndName(vendorId, name) {
        const document = await VenueModel.findOne({ vendorId, name });
        if (!document) return null;
        return VenueMapper.mapToEntity(document);
    }
    ...
    async findAllFiltered(query = {}) {
        ...
        if (query.search) {
            filter.$or = [
                ...
                { "address.addressLine1": { $regex: query.search, $options: "i" } },
        ...
        return { data: documents, totalCount, totalPages };
    }

    async approveVenue(id) {
        const venue = await VenueModel.findByIdAndUpdate(
            id,
            { approvalStatus: VenueApprovalStatus.APPROVED, rejectionReason: null },
            { new: true }
    ...
    async rejectVenue(id, reason) {
        const venue = await VenueModel.findByIdAndUpdate(
            id,
            { approvalStatus: VenueApprovalStatus.REJECTED, rejectionReason: reason },
            { new: true }
        ).populate("vendorId", "fullName email");
        if (!venue) return null;
        return VenueMapper.mapToEntity(venue);
    ...
    async countByOwnerId(ownerId) {
        return await VenueModel.countDocuments({ ownerId, isDeleted: false });
    }
}
