...
function AdminDashboard({ user, onLogout }) {
  ...
  const [allVenues, setAllVenues]         = useState([]);  // ✅ NEW
  ...
  // ✅ NEW: Fetch all bookings — GET /api/admin/bookings
  ...
  // ✅ NEW: Fetch all venues — GET /api/admin/venues
  const fetchAllVenues = async () => {
    try {
      const res = await fetch('http://localhost:8080/api/admin/venues', {
        headers: { Authorization: `Bearer ${getToken()}` },
      });
      if (!res.ok) throw new Error('Failed to fetch all venues');
      setAllVenues(await res.json());
    } catch (err) { console.error(err); }
  };
  ...
  const handleConfirmReview = async () => {
    ...
    try {
      ...
      closeModal();
      await fetchPendingVenues();
      await fetchAllVenues(); // ✅ refresh all venues tab too
    } catch (err) { toast.error(err.message); }
  };

  // ✅ NEW: Update booking status — PATCH /api/admin/booking/{id}/status?bookingStatus=
  ...
  // ✅ NEW: Update payment status — PATCH /api/admin/booking/{id}/payment/status?paymentStatus=
  ...
  // ✅ NEW: Update venue status — PATCH /api/admin/venue/{id}/status?status=
    ...
    <div className="dashboard-container">
      ...
      <header className="dashboard-nav">
        ...
        <div className="dashboard-tabs">
          ...
          <button
            className={`tab-button ${activeTab === 'all-venues' ? 'active' : ''}`}
      ...
      <main className="dashboard-content-area">
        ...
        <div className="metrics-row">
          ...
          <div className="metric-card">
            ...
            <span className="metric-val">{allBookings.length}</span>
          </div>
        </div>
        ...
        {/* ════════════════════════════════════════════════
            TAB 1 — USER MANAGEMENT (unchanged)
        ════════════════════════════════════════════════ */}
        ...
        {/* ════════════════════════════════════════════════
            TAB 3 — ALL BOOKINGS ✅ NEW
            GET /api/admin/bookings
            PATCH /api/admin/booking/{id}/status
            PATCH /api/admin/booking/{id}/payment/status
        ════════════════════════════════════════════════ */}
        ...
        {/* ════════════════════════════════════════════════
            TAB 4 — ALL VENUES ✅ NEW
            GET /api/admin/venues
            PATCH /api/admin/venue/{id}/status
        ════════════════════════════════════════════════ */}
                  ...
                  <tbody>
                    {allVenues.map(venue => (
                      <tr key={venue.id}>
                        ...
                        {/* Status dropdown — PATCH /api/admin/venue/{id}/status */}
                        <td>
                          <select
                            style={selectStyle}
                            value={venue.status}
