...
  // null = closed, object = { bookingId, venueName, action: 'APPROVED'|'REJECTED' }
...
  // GET /api/owner/bookings — returns all bookings for venues owned by this owner
  const fetchBookingRequests = async () => {
    setRequestsLoading(true);
    try {
      const response = await fetch('http://localhost:8080/api/owner/bookings/reviews', {
        headers: { Authorization: `Bearer ${getToken()}` },
      });
      if (!response.ok) throw new Error('Failed to load booking requests');
      setBookingRequests(await response.json());
    } catch (error) {
      console.error(error);
...
  // Calls PATCH /api/owner/bookings/{id}/review
  const handleConfirmReview = async () => {
    if (reviewModal.action === 'REJECTED' && !ownerComment.trim()) {
...
        `http://localhost:8080/api/owner/bookings/${reviewModal.bookingId}/review`,
        {
          method: 'PATCH',
          headers: {
...
          },
          body: JSON.stringify({
            bookingStatus: reviewModal.action,   // "APPROVED" or "REJECTED"
            ownerComments: ownerComment,          // message to customer
          }),
        }
      );

      // Read response safely — backend might return text or JSON
      const text = await res.text();
      if (!res.ok) {
        try {
          const data = JSON.parse(text);
          throw new Error(data.error || data.message || 'Failed to review booking');
        } catch {
          throw new Error(text || 'Failed to review booking');
        }
      }

      toast.success(
        `Booking ${reviewModal.action === 'APPROVED' ? 'approved' : 'rejected'} successfully!`
      );
      closeReviewModal();
      await fetchBookingRequests(); // refresh list
...
        },
        body: JSON.stringify({
          venueName: name, venueType: type, location,
          venueDescription: description,
          capacity: Number(capacity), price: Number(price),
          parkingAvailable: parking, imageUrl: image, termsAccepted,
        }),
      });
      if (!response.ok) throw new Error('Failed to submit venue');
      toast.success('Venue request submitted! Awaiting admin approval.');
      await fetchVenues();
    } catch (error) {
...
        `http://localhost:8080/api/owner/delete/venue/${venueId}`,
...
    try {
      const response = await fetch(
        `http://localhost:8080/api/owner/update/venue/${editingVenue.id}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
...
          },
          body: JSON.stringify({
            venueName: editName, venueType: editType,
            location: editLocation, venueDescription: editDescription,
            capacity: Number(editCapacity), price: Number(editPrice),
            parkingAvailable: editParking, imageUrl: editImage,
          }),
        }
      );
      if (!response.ok) throw new Error('Failed to update venue');
      toast.success('Venue updated! Status reset to PENDING for re-approval.');
...
  const pendingRequests  = bookingRequests.filter(b => b.bookingStatus === 'PENDING').length;
  const approvedRequests = bookingRequests.filter(b => b.bookingStatus === 'APPROVED').length;
...
                            </div>
                          ) : (
                            // Show owner comment if already reviewed
...
              {/* Buttons — uses your existing .modal-actions CSS */}
              <div className="modal-actions">
