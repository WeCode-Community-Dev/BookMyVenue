...
function VenueDetails() {
  ...
  // ── Calendar month tracking for availability API ──
  ...
  const imageAlt = (img) =>
    typeof img === "string" ? venue?.name || "Venue image" : img?.alt || venue?.name || "Venue image";
  ...
  const getPricingRow = (dayType) =>
    pricingMap[dayType] || pricingRows[0] || { price: 0, minHours: venue?.minBookingHours || 1 };
  ...
  const priceEstimate = useMemo(() => {
    if (bookingType === "hourly") {
      ...
      return { amount: hours * Number(pricing.price || 0), hours, dayType: getDayType(date), pricing };
    }
    ...
    if (bookingType === "daily") {
      ...
      while (current <= end) {
        ...
        dayCount += 1;
        breakdown.push({ date: current.toISOString().slice(0, 10), dayType, amount: Number(pricing.price || 0) });
        current.setDate(current.getDate() + 1);
      }
  ...
  if (isLoading) {
    return <PageTransition className="venue-details-page"><VenueDetailSkeleton /></PageTransition>;
  }
  if (error) {
    return (
      <PageTransition className="venue-details-page">
        <main className="container container--centered">
          <EmptyState title="Unable to load venue" message="Something went wrong while fetching venue details. Please try again later." variant="error" />
        </main>
      </PageTransition>
    );
  }
  if (!venue) {
    return (
      <PageTransition className="venue-details-page">
        <main className="container container--centered">
          <EmptyState title="Venue not found" message="The venue you're looking for doesn't exist or may have been removed." actionLabel="Browse Venues" actionTo="/browse-venues" />
        </main>
      </PageTransition>
    );
  }
  ...
  return (
    <PageTransition className="venue-details-page">
      ...
      <section className="venue-gallery">
        <div className="venue-gallery__grid" data-count={Math.min(images.length, 5)}>
          <div className="venue-gallery__main" onClick={() => openLightbox(0)} role="button" tabIndex={0}>
            <img src={imageUrl(images[0] || {})} alt={imageAlt(images[0] || {})} />
            <div className="venue-gallery__overlay"><FiMaximize2 /></div>
          </div>
          {images.slice(1, 5).map((img, index) => (
            <div key={index + 1} className="venue-gallery__thumb" onClick={() => openLightbox(index + 1)} role="button" tabIndex={0}>
              <img src={imageUrl(img)} alt={imageAlt(img)} />
              <div className="venue-gallery__overlay"><FiMaximize2 /></div>
              {index === 3 && images.length > 5 && (
                <div className="venue-gallery__more"><span>+{images.length - 5}</span><small>more photos</small></div>
              )}
            </div>
          ))}
        </div>
        <div className="venue-gallery__info">
          <div className="venue-gallery__info-inner">
            <div className="venue-gallery__meta">
              ...
              <p className="venue-gallery__location"><FiMapPin /> {venueCity}</p>
            </div>
            <div className="venue-gallery__actions">
              <button type="button" className={`favorite-toggle ${isFavorited ? "active" : ""}`} onClick={toggleFavorite} aria-label="Toggle favorite">
                <FiStar /><span>{isFavorited ? "Saved" : "Save"}</span>
              </button>
            </div>
          </div>
        </div>
      ...
      {lightboxOpen && (
        <div className="lightbox" onClick={closeLightbox}>
          <div className="lightbox__inner" onClick={(e) => e.stopPropagation()}>
            <button className="lightbox__close" onClick={closeLightbox} aria-label="Close gallery"><FiX /></button>
            <div className="lightbox__stage">
              {images.length > 1 && <button className="lightbox__nav lightbox__nav--prev" onClick={lightboxPrev} aria-label="Previous image"><FiChevronLeft /></button>}
              <img className="lightbox__image" src={imageUrl(images[lightboxIndex] || {})} alt={imageAlt(images[lightboxIndex] || {})} />
              {images.length > 1 && <button className="lightbox__nav lightbox__nav--next" onClick={lightboxNext} aria-label="Next image"><FiChevronRight /></button>}
            </div>
            <div className="lightbox__counter">{lightboxIndex + 1} / {images.length}</div>
            {images.length > 1 && (
              <div className="lightbox__thumbs">
                {images.map((img, idx) => (
                  <button key={idx} type="button" className={`lightbox__thumb ${idx === lightboxIndex ? "active" : ""}`} onClick={() => setLightboxIndex(idx)}>
                    <img src={imageUrl(img)} alt={imageAlt(img)} />
                  </button>
                ))}
              </div>
      ...
      <div className="venue-stats-bar">
        <div className="venue-stats-bar__inner">
          <div className="venue-stat">
            ...
            <div><span className="venue-stat__label">Capacity</span><span className="venue-stat__value">Up to {venue.capacity}</span></div>
          </div>
          <div className="venue-stat">
            <FiClock className="venue-stat__icon" />
            <div><span className="venue-stat__label">Hours</span><span className="venue-stat__value">{venue.openTime || "—"} – {venue.closeTime || "—"}</span></div>
          </div>
          <div className="venue-stat">
            <FiCalendar className="venue-stat__icon" />
            <div><span className="venue-stat__label">Booking</span><span className="venue-stat__value">{bookingLabel}</span></div>
          </div>
          {startingPrice != null && (
      ...
      <main className="container">
        <section className="main-content">
          <div className="detail-card detail-card--compact">
            <div className="detail-card__header"><span className="eyebrow">Schedule</span><h2>Open days</h2></div>
          ...
          <div className="content-section detail-card">
            <div className="detail-card__header"><span className="eyebrow">Overview</span><h2>About this venue</h2></div>
            <p className="about-text">{venue.description}</p>
            <div className="venue-address">
              <FiMapPin className="venue-address__icon" />
              <div><strong>Address</strong><p>{venue.address}{venue.pincode ? ` • ${venue.pincode}` : ""}</p></div>
            </div>
          </div>

          <div className="content-section detail-card">
            <div className="detail-card__header"><span className="eyebrow">Features</span><h2>Amenities</h2></div>
            <div className="amenities-container">
              ...
              {Object.keys(amenitiesByCategory).length === 0 && <p className="empty-hint">No amenities listed for this venue.</p>}
            </div>
          </div>

          <div className="content-section detail-card">
            <div className="detail-card__header"><span className="eyebrow">Hours</span><h2>Availability</h2></div>
            <div className="info-row">
              <div className="info-block"><span className="info-label">Opens</span><span className="info-value">{venue.openTime || "N/A"}</span></div>
              <div className="info-block"><span className="info-label">Closes</span><span className="info-value">{venue.closeTime || "N/A"}</span></div>
              <div className="info-block"><span className="info-label">Min. booking</span><span className="info-value">{minBookingHours} {minBookingHours === 1 ? "hr" : "hrs"}</span></div>
            </div>
          </div>
        </section>
        ...
        <aside className="sidebar-wrapper">
          <div className="booking-card">
            <div className="booking-card__header"><span className="eyebrow">Reserve</span><h2>Book this venue</h2></div>
            <div className="booking-type-badge"><FiCalendar /><span>{bookingLabel}</span></div>
            <div className="pricing-cards">
              {pricingRows.length > 0 ? (
                pricingRows.map((pricing) => (
                  <div key={`${pricing.dayType}-${pricing.id || pricing.dayType}`} className="pricing-card">
                    <span className="pricing-card__day">{pricing.dayType?.charAt(0).toUpperCase() + pricing.dayType?.slice(1)}</span>
                ...
                <>
                  ...
                  <div className="form-row">
                    <div className="form-group">
                      ...
                      <select id="start-time" className="form-input" value={startTime} onChange={(e) => setStartTime(e.target.value)}>
                    ...
                    <div className="form-group">
                      ...
                      <select id="end-time" className="form-input" value={endTime} onChange={(e) => setEndTime(e.target.value)}>
                        <option value="10:00">10:00 AM</option>
                        <option value="11:00">11:00 AM</option>
                        <option value="12:00">12:00 PM</option>
                        <option value="13:00">01:00 PM</option>
                  ...
                  <div className="estimation-row">
                    ...
                    <span className="est-value">{priceEstimate ? formatCurrency(priceEstimate.amount) : "—"}</span>
                  </div>
                  {!priceEstimate && <p className="est-hint">Select a date and time to see pricing</p>}
                </>
              ) : (
                <>
                  <div className="form-group">
                    ...
                    <DayPicker
                      ...
                      selected={{
                        from: bookingStartDate ? new Date(bookingStartDate) : undefined,
                        to: bookingEndDate ? new Date(bookingEndDate) : undefined,
                      }}
                      onSelect={(range) => {
                        setBookingStartDate(range?.from ? range.from.toISOString().slice(0, 10) : "");
                        setBookingEndDate(range?.to ? range.to.toISOString().slice(0, 10) : "");
                      }}
                  ...
                  {!priceEstimate && <p className="est-hint">Select booking dates to see pricing</p>}
                </>
              )}
            </div>
            ...
            <div className="booking-card__footer">
              ...
              <span className="muted-note">
                {bookingType === "hourly" ? `Min. booking: ${minBookingHours} ${minBookingHours === 1 ? "hour" : "hours"}` : "Daily bookings use a date range."}
              </span>
            </div>
          </div>
        </aside>
