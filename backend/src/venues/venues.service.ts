...
export class VenuesService {
  ...
  async update(id: string, updateVenueDto: UpdateVenueDto, userId: string, userRole: string): Promise<Venue | null> {
    const venue = await this.venueModel.findById(id).exec();
    if (!venue) {
      return null;
    }

    if (userRole !== 'admin' && venue.ownerId.toString() !== userId) {
      throw new ForbiddenException('You do not have permission to update this venue');
    }

    const savedImages: string[] = [];
    if (updateVenueDto.images) {
      for (const imageStr of updateVenueDto.images) {
        if (imageStr.startsWith('data:image/')) {
          const matches = imageStr.match(/^data:image\/([A-Za-z0-9\-+]+);base64,(.+)$/);
          if (matches && matches.length === 3) {
            const ext = matches[1];
    ...
    return this.venueModel.findByIdAndUpdate(id, updateData, { new: true }).exec();
  }
  ...
  async remove(id: string, userId: string, userRole: string): Promise<boolean> {
    ...
    if (userRole !== UserRole.ADMIN && venue.ownerId.toString() !== userId) {
